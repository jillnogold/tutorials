...
if [[ "${tag_prefix}" < "1.6" ]]; then
    error_exit "MLRun version must be 1.7.0 or above, for older updates run: sh https://raw.githubusercontent.com/mlrun/demos/v1.6.0/update_demos.sh .Aborting..."
fi

# removing old demos folder
dt=$(date '+%Y%m%d%H%M%S');
